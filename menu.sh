...
async fn handle_client(mut client_stream: TcpStream) -> Result<(), Error> {
    ...
    if let Err(e) = server_connect {
        println!("ERRO AO CONECTAR AO PROXY: {}", e);
        return Ok(());
    }
    ...
    println!("Transferência concluída, aguardando antes de encerrar...");
    tokio::time::sleep(Duration::from_secs(1)).await;
    println!("Encerrando conexão.");
...
fn get_port() -> u16 {
    ...
    for i in 1..args.len() {
        if args[i] == "--port" {
            if i + 1 < args.len() {
                port = args[i + 1].parse().ok().unwrap_or(80);
            }
        }
    }
